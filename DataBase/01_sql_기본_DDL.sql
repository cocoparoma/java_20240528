...
create table if not exists student2.score(
    ...
    sc_preformance int not null default 0,
	sc_st_key int not null,
    sc_su_key int not null,
    foreign key(sc_st_key) references student(st_key) on delete cascade on update cascade,
    foreign key(sc_su_key) references subject(su_key) on delete cascade);

select * from subject;
insert into student(st_grade, st_class, st_num, st_name) values (1, 2, 3, "안철수");
delete from student where st_key = 2;