...
public class PageMaker {
	...
	private int displayPageNum; // 한 페이지 네이션의 페이지 개수
	private boolean prev; // 이전버튼 활성화
	private boolean next; // 다음벌튼 활성화
	private Criteria cri; // 현 페이지 정보

	// 시작페이지, 마지막 페이지, 이전버튼 활성화여부, 다음 버튼 활성화 여부를 계산하는 메소드
	public void calculte() {
		// 마지막 페이지 번호를 계산 => 최대 페이지 번호
		endPage = (int)(Math.ceil(cri.getPage()/(double)displayPageNum) * displayPageNum);
		
		startPage = endPage -displayPageNum + 1;
		// 마지막 페이지 네이션에서 마지막 페이지 번호
		int lastEndPage = (int)(Math.ceil(totalCount/(double)cri.getPerPageNum()));
		
		if(endPage > lastEndPage) {
			endPage = lastEndPage;
		}
		// 이전 다음 버튼을 한 페이지 네이션 단위로 이동 (콘솔에서 이용하기 어려움)
