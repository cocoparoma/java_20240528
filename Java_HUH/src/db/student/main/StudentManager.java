...
public class StudentManager{
	...
	public void init() {
		while (true) {
			...
			else if (M.equals("2")) subjectUpdateinit();//subject 관리
	...
	public void insert() {//Student 관리 메뉴
		while (true) {
			...
			if (M.equals("1")) studentController.insertStudent();//Student 관리
			else if (M.equals("2")) studentUpdateinit();// 학생 수정 매뉴
			else if (M.equals("3")) studentController.deleteStudent();
			else if (M.equals("4")) studentController.selectStudent();
	...
	public void subjectUpdateinit() {//subject menu임
		while (true) {
			System.out.print("[과목 관리 메뉴]\n[1]과목 추가\n[2]과목 수정\n[3]과목 삭제\n[4]과목 조회\n[5]뒤로가기\n>>");
			
			String M = sc.nextLine();
			//subject관련으로 바꿀 것
			if (M.equals("1")) subjectController.insertSubject();
			else if (M.equals("2")) subjectController.updateSubject();
			else if (M.equals("3")) subjectController.deleteSubject();
			else if (M.equals("4")) subjectController.selectSubject();
			else if (M.equals("5")) break;
			else if (M.equals("show")) showAll();
			else System.out.println("[잘못된 입력]");
		}
	...
	private void updateSubjectScore() {
		...
		SubjectVO sub = new SubjectVO(subject, sgrade, semester, midterm, finals, Perfomace);
	...
	private void deleteSubjectScore() {
		...
		SubjectVO sub = new SubjectVO(subject, sgrade, semester,0,0,0);
	...
	private SubjectVO inputSubject() {
		...
		return new SubjectVO(subject, sgrade, semester, midterm, finals, Perfomace);
	}
	...
	private void insertSubject(int a) {
		...
		subjectList.add(new SubjectVO(subString));
	...
	public StudentVO inputStudent () {
		...
		return 	new StudentVO(" ", tmpGrade, tmpClassNumber, tmpNumber);
	}
	public StudentVO inputstudentE() { //추가적으로 이름까지 입력
		StudentVO std =  inputStudent () ;
		System.out.print("[이름]\n>>");
	...
	private int sameStudent(StudentVO std) {//학생 중복확인
...
//student 관리 ******
	private void studentInsert(int a) {//수정은 사용되지 아니하였음
		...
		StudentVO stdStudent = inputstudentE();
	...
	private void studentUpdate() {//수정, 중복확인, 없는정보도 완료됨
		StudentVO ste = studentSearch(0); //수정할 정보 찾아옴, 오류가 있으면 -10으로 들어가 있어서 찾기 편함(같을 수가없음)
		if (ste == null) return; //가지고 온 정보가 null 이면 리턴 즉 없는 정보를 받아왔음 안내문구는 위에서 해줌
		StudentVO tmpStudent = inputstudentE(); // 변경할 정보 만들어옴
	...
	private void studentDelete() {//삭제
		StudentVO ste = studentSearch(0);
		list.remove(ste);
		System.out.println("[삭제 완료]");
	}

	private StudentVO studentSearch(int a) {//찾기 >> 수정과 삭제에 사용되었음 a가 4로 입력되면 추가사항없이 출력만 하도록 설계됨 / 잘못 입력시 널값을 가져가게 됨 / 널포인트에러가 날 가능성 O <<널 찾기놀이 중요
		System.out.print("[찾으시는 학생의 이름을 입력해 주세요.]\n>>");
		String tmpName =sc.nextLine();
		List<StudentVO> tmplist = list.stream().filter(p->p.getName().contains(tmpName)).toList();
		if (tmplist.size() == 0) {
		...
		StudentVO tmpStudent = tmplist.get(idx-1);
		return tmpStudent;
	}
	//student 관리 done******
